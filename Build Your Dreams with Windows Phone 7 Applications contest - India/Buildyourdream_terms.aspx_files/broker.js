...
var _previewsite = ['/en-us/(default\\.aspx)?$', '/zh-cn/(default\\.aspx)?$', '/en-gb/(default\\.aspx)?$', '/fr-fr/(default\\.aspx)?$', '/de-de/(default\\.aspx)?$'];
...
for (var i=0; i <_previewsite.length; i++)
{
	_sr_start = "//[\\w\\.-]+";
	if(SR_url_stripped[0].search(_sr_start + _previewsite[i]) != -1) {
	
	_sr_config = "broker-config_p.js";
	
	}
}

if(SR_url_stripped[0].search('//[\\w\\.-]+/en-us/cloud/(?!developer)') != -1) {
	_sr_config = "broker-config_cloud.js";
	_sr_builder= "builder_cloud.js";
}
