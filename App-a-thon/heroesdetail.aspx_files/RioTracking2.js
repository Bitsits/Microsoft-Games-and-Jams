...
RioTrackingManager.prototype.setCellCode=function(){this.crcc=escape(this.queryStringParamSafeValue("CR_CC",true,this.guestCellCode));if(this.crcc==this.guestCellCode){var currentCc=this.getCookie(this.cellCodeCookieKey);if((currentCc!=null)&&(currentCc.toString().length>0)&&(currentCc.split('.')[1]=='1')){this.crcc=parseInt(currentCc.split('.')[0]);}}
...
RioTrackingManager.prototype.setUserId=function(){this.crid=escape(this.queryStringParamSafeValue("CR_ID",true,-1));if(this.crid==-1){var currentId=this.getCookie(this.permIdCookieKey);if((currentId!=null)&&(currentId.toString().length>0)){this.crid=currentId;}}
document.cookie=this.permIdCookieKey+"="+this.crid;}
RioTrackingManager.prototype.fireImpressionTags=function(){if(this.uniqueActionTagCode!="null"){this.fireAtlasImpressionTag();}
if((this.crcc!="-1")&&(this.fireCrsImpressionTags)){this.fireCrsImpressionTag();}}
RioTrackingManager.prototype.click=function(endActionCode,atlasTagCode){if(atlasTagCode!=null){this.fireAtlasClickTag(atlasTagCode,endActionCode);}
this.fireCrsClickTag(endActionCode);var date=new Date();var curDate=null
do{curDate=new Date();}
while(curDate-date<500);return true;}
RioTrackingManager.prototype.clickChatDirect=function(endActionCode,atlasTagCode){this.isChat=true;this.click(endActionCode,atlasTagCode);return true;}
RioTrackingManager.prototype.clickChatProactive=function(endActionCode,atlasTagCode){this.isChat=true;this.isProactiveChat=true;this.click(endActionCode,atlasTagCode);return true;}
RioTrackingManager.prototype.atlasTrack=function(atlasTagCode){atlasUrl=this.baseAtlasUrlNotExtended.format(atlasTagCode);this.fireTag(atlasUrl,true);var date=new Date();var curDate=null
do{curDate=new Date();}
while(curDate-date<500);return true;}
RioTrackingManager.prototype.clickNoWait=function(endActionCode,atlasTagCode){if(atlasTagCode!=null){this.fireAtlasClickTag(atlasTagCode,endActionCode);}
this.fireCrsClickTag(endActionCode);}
RioTrackingManager.prototype.trackVideoEvent=function(endActionCode,eventId){var atlasUrl=this.baseOnvAtlasUrl.format(this.crid,this.crcc,endActionCode,eventId);var crsUrl=this.baseOnvEventUrl.format(this.crcc,eventId.toString(),endActionCode.toString(),this.crid);var timestamp=new Date();crsUrl+="&rnd="+Math.ceil(Math.random()*99999999)+""+timestamp.getUTCFullYear()+timestamp.getUTCMonth()+timestamp.getUTCDate()+timestamp.getUTCHours()+timestamp.getUTCMinutes()+timestamp.getUTCSeconds()+timestamp.getUTCMilliseconds();crsUrl+="&r="+escape(document.referrer);this.fireTag(atlasUrl,false);this.fireTag(crsUrl,true);}
RioTrackingManager.prototype.trackMeteorReferral=function(appId,referringFbid){this.crcc=escape(this.queryStringParamSafeValue("CR_CC",true,this.guestCellCode));var meteorReferralUrl=this.baseMeteorReferralUrl.format(this.crcc,referringFbid,appId);this.fireTag(meteorReferralUrl,false);}
RioTrackingManager.prototype.fireAtlasImpressionTag=function(){var atlasUrl;if(this.isExtendedDataTag){atlasUrl=this.baseAtlasUrl.format(this.uniqueActionTagCode,this.crid,this.crcc,this.eac);}
else{atlasUrl=this.baseAtlasUrlNotExtended.format(this.uniqueActionTagCode);}
this.fireTag(atlasUrl,false);}
RioTrackingManager.prototype.fireCrsImpressionTag=function(){var crsUrl=this.baseCrsImpressionUrl+"?CR_CC="+this.crcc;if((this.vn!=-1)&&(this.fn!=-1)){crsUrl+="&vn="+this.vn.toString()+"&fn="+this.fn.toString();}
...
RioTrackingManager.prototype.fireAtlasClickTag=function(atlasUatc,endActionCode){var atlasUrl=this.baseAtlasUrl.format(atlasUatc,this.crid,this.crcc,endActionCode);this.fireTag(atlasUrl,true);}
RioTrackingManager.prototype.fireCrsClickTag=function(endActionCode){document.cookie=this.endActionCodeCookieKey+"="+endActionCode.toString();var crsUrl=this.baseCrsClickUrl;crsUrl+="?CR_EAC="+endActionCode.toString();if(this.testMode){crsUrl+="&test=1";}
...
RioTrackingManager.prototype.setWtCookie=function(){var cookieVal="";try{var rCookie=this.getCookie(this.wtCookieKey);var cookies;if(rCookie!=''){cookies=this.getCookie(this.wtCookieKey).split("|");}
...
var newCellCodeVal=currentCellCode+"-"+RioTracking.getCurrentDateTime();cookies=this.ArrayAdd(cookies,0,newCellCodeVal);var newCookie=((cookies.length>1?this.ArrayJoin(cookies,3,"|"):cookies));var exdate=new Date();exdate.setDate(exdate.getFullYear()+20);cookieVal=this.wtCookieKey+"="+newCookie+";expires="+exdate.toGMTString()+";domain=microsoft.com;path=/";document.cookie=cookieVal;this.isWtCookieSet=true;}
catch(e){cookieVal="";}
return cookieVal;}
RioTrackingManager.prototype.AddHandler=function(object,eventName,handler){if(object.addEventListener){object.addEventListener(eventName,handler,false);}
else{object.attachEvent("on"+eventName,handler);}}
RioTrackingManager.prototype.CreateDelegate=function(object,method){return(function(){return method.apply(object,arguments);})}
RioTrackingManager.prototype.fireTag=function(tag,appendRandom){var scriptObj=document.createElement("script");scriptObj.type="text/javascript";scriptObj.src=tag;document.getElementsByTagName("head")[0].appendChild(scriptObj);}
RioTrackingManager.prototype.queryStringParamSafeValue=function(name,caseInsensitive,defaultValueIfNull){name=name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var r="[?&]"+name+"=([^&#]*)";var rmod=""
...
RioTrackingManager.prototype.getCookie=function(check_name){var a_all_cookies=document.cookie.split(';');var a_temp_cookie='';var cookie_name='';var cookie_value='';var b_cookie_found=false;for(i=0;i<a_all_cookies.length;i++){a_temp_cookie=a_all_cookies[i].split('=');cookie_name=a_temp_cookie[0].replace(/^\s+|\s+$/g,'');if(cookie_name==check_name){b_cookie_found=true;if(a_temp_cookie.length>1){cookie_value=unescape(a_temp_cookie[1].replace(/^\s+|\s+$/g,''));}
...
RioTrackingManager.prototype.ArrayIndexOf=function(array,obj){var i=array.length;while(i--){if(array[i].match(obj+"-")){return i;}}
return-1;}
RioTrackingManager.prototype.ArrayJoin=function(array,count,delimiter){var _result="";var _maxCellCodes=(count>array.length?array.length:count);for(_index=0;_index<_maxCellCodes;_index++){_result+=array[_index]+(_index<_maxCellCodes-1?delimiter:"");}
return _result;}
RioTrackingManager.prototype.ArrayAdd=function(array,addIndex,obj){for(_index=array.length;_index>addIndex;_index--){array[_index]=array[_index-1];}
array[addIndex]=obj;return array;}
RioTrackingManager.prototype.getCurrentDateTime=function(){var currentTime=new Date();var month=currentTime.getMonth()+1;var day=currentTime.getDate();var year=currentTime.getFullYear();var hours=currentTime.getHours();var minutes=currentTime.getMinutes();var seconds=currentTime.getSeconds();if(minutes<10){minutes="0"+minutes;}
if(seconds<10){seconds="0"+seconds;}
return month+"/"+day+"/"+year+" "+hours+":"+minutes+":"+seconds;}
var RioTracking=new RioTrackingManager();if(typeof(meteorAppId)!='undefined'&&meteorAppId.length!=0){var meteorRedirectCookie=RioTracking.getCookie(RioTracking.meteorRedirectCookieKey);var fbid=RioTracking.queryStringParamSafeValue("fbid",true,-1).toString();if((meteorRedirectCookie==null)||(meteorRedirectCookie.toString().length==0)){if(fbid!=-1){document.cookie=RioTracking.referringFbidCookieKey+"="+RioTracking.queryStringParamSafeValue("fbid",true,-1).toString();document.cookie=RioTracking.meteorRedirectCookieKey+"=1";}
